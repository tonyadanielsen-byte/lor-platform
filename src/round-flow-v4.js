import { createRoundV4, saveResponseV4, addEmployeeInterviewV4, completeRoundV4 } from './store-v4.js';
import { availableThemesV4 } from './seed-data-v4.js';
...
export function createRoundControllerV4({ user, onDone, notify, themeBank, initialPlan = null, backdateMode = false }) {
  let roundId = null;
  let step = -1;
  let plan = initialPlan;
  let department = plan?.department || 'Renhold';
  let themes = availableThemesV4(themeBank, department);
  let theme = themes.find(t => t.name === (plan?.theme || plan?.themeName)) || themes[0];
  let roundDate = plan?.plannedDate || today();
  const responses = {};
  ...
  function refreshThemes(preferredName = '') {
    themes = availableThemesV4(themeBank, department);
  ...
  async function ensureRound() {
    ...
    if (!roundDate) throw new Error('Velg dato for LOR-runden.');

    roundId = await createRoundV4({
      planId: plan?.id || null,
      sourceSeedId: plan?.sourceSeedId || (plan?.source === 'seed' ? plan.id : ''),
      planWeek: plan?.week || null,
  ...
  function findings() {
    return (theme?.questions || [])
      .map(q => ({ q, response: responses[q.id] }))
      .filter(x => x.response && ['improvement','deviation'].includes(x.response.status));
  }

  function intro() {
    const historical = roundDate && roundDate < today();
    const title = backdateMode ? 'Registrer LOR tilbake i tid' : (plan ? `Uke ${esc(plan.week)} · ${esc(plan.theme || plan.themeName || 'LOR')}` : 'Start LOR');
    return `
      <section class="round-shell card round-intro-v2">
        <div class="round-head"><div><span class="eyebrow">${backdateMode ? 'Tilbakeført registrering' : 'Ny lederoppfølgingsrunde'}</span><h1>${title}</h1><p>${backdateMode ? 'Velg faktisk dato runden ble gjennomført. Uke og år lagres automatisk fra datoen.' : (plan ? 'Planlagt runde er forhåndsutfylt. Kontroller dato og start.' : 'Velg avdeling, tema og dato. Spørsmålene hentes fra LOR-banken.')}</p></div><span class="round-state">${historical ? 'Tilbakeføres' : 'Klar'}</span></div>
        ...
        <div class="form-grid">
          <label>Dato for runden<input id="roundDate" type="date" required max="${today()}" value="${esc(roundDate)}"></label>
        ...
        <div class="positive-start"><div class="positive-icon">★</div><div><strong>Start med det som fungerer</strong><p>Registrer én konkret positiv observasjon før kontrollpunktene.</p></div><textarea id="positiveStart" placeholder="Hva gjorde medarbeiderne eller området bra?">${esc(positiveStart)}</textarea></div>
        <div class="round-meta-line"><span>${theme?.questions?.length || 0} kontrollpunkter</span><span>${esc(user.name)}</span><span>${esc(roundDate || '')}</span>${plan?.week ? `<span>Planuke ${esc(plan.week)}</span>` : ''}</div>
        <button class="primary-action full-action" data-round-action="start">${backdateMode ? 'Start tilbakeført registrering' : 'Start runden'}</button>
      </section>`;
  }
  ...
  function question() {
    ...
    return `
      <section class="round-shell card question-shell-v2">
        ...
        <div class="question-context"><span>${esc(theme.name)}</span><span>${esc(department)}</span><span>${esc(roundDate)}</span></div>
  ...
  function interview() {
    return `
      <section class="round-shell card interview-v2">
        <div class="round-head"><div><span class="eyebrow">Medarbeiderdialog</span><h1>Hva sier medarbeideren?</h1><p>Registrer bare det som er relevant og nyttig.</p></div><span class="round-step-chip">Frivillig</span></div>
        ...
        <div class="interview-grid">
          <label class="stack"><span>Hvordan oppleves arbeidshverdagen?</span><textarea id="employeeEnvironment"></textarea></label>
          <label class="stack positive-field"><span>Hva fungerer bra?</span><textarea id="employeePositive"></textarea></label>
          <label class="stack improvement-field full"><span>Hva bør vi forbedre?</span><textarea id="employeeImprove"></textarea></label>
        </div>
        <label class="followup-toggle"><input type="checkbox" id="employeeFollowUp"><span><strong>Dette innspillet krever oppfølging</strong></span></label>
  ...
  function summary() {
    ...
    return `
      <section class="round-shell card summary-v2">
        <div class="round-head"><div><span class="eyebrow">Oppsummering</span><h1>Runden er klar</h1><p>Registreres på dato <strong>${esc(roundDate)}</strong>.</p></div><span class="round-state">Klar til lagring</span></div>
        ...
        <div class="summary-section"><div class="summary-title"><span>FUNN OG OPPFØLGING</span><strong>${issueRows.length + (employeeInterview?.needsFollowUp ? 1 : 0)}</strong></div>${issueRows.length ? `<div class="finding-list">${issueRows.map(({q,response}) => `<div class="finding-row ${response.status}"><div><strong>${response.status==='deviation'?'Avvik':'Forbedringspunkt'}</strong><p>${esc(q.text)}</p>${response.comment ? `<small>${esc(response.comment)}</small>` : ''}</div></div>`).join('')}</div>` : '<p class="muted">Ingen funn krever oppfølging.</p>'}</div>
        <label class="stack">Oppsummering / neste steg<textarea id="roundSummary">${esc(summaryText)}</textarea></label>
  ...
  function handleChange(target) {
    ...
    if (target.id === 'roundDate') {
      roundDate = target.value;
      return false;
    }
  ...
  async function handle(target, root) {
    ...
    if (action === 'start') {
      roundDate = root.querySelector('#roundDate')?.value || roundDate;
      ...
      if (!roundDate) { notify('Velg dato for runden.', true); return { rerender:false }; }
      if (roundDate > today()) { notify('Dato kan ikke være frem i tid.', true); return { rerender:false }; }
      ...
      await saveResponseV4(await ensureRound(), q.id, response);
      step += 1;
    } else if (action === 'back') {
      ...
      if (!employeeInterview.environment && !employeeInterview.positive && !employeeInterview.improvement) {
        notify('Registrer minst ett innspill, eller velg Hopp over.', true);
        return { rerender:false };
      }
      await addEmployeeInterviewV4(await ensureRound(), employeeInterview);
      step += 1;
      ...
      const counts = statusCounts();
      const followUpCount = (counts.improvement || 0) + (counts.deviation || 0) + (employeeInterview?.needsFollowUp ? 1 : 0);
      await completeRoundV4(await ensureRound(), {
        text: summaryText,
        counts,
        needsFollowUp: Boolean(followUpCount),
        followUpCount
      }, roundDate);
      notify(`LOR ${roundDate} er lagret ✓`);
      onDone?.(roundId);
    }
    return { rerender: true };
  }
