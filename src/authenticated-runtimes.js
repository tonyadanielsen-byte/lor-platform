...
async function loadAuthenticatedRuntimes() {
  ...
  await import('./v363-guard.js?v=3.8.15');
  await import('./v374-hotfix.js?v=3.8.15');

  document.documentElement.dataset.authenticatedRuntimes = '3.8.15';
}

if (!auth) {
  console.error('[LOR 3.8.15] Firebase Auth er ikke tilgjengelig for runtime-loader.');
} else {
  auth.onAuthStateChanged(user => {
    if (user) {
      loadAuthenticatedRuntimes().catch(error => {
        runtimesLoaded = false;
        console.error('[LOR 3.8.15] Kunne ikke laste autentiserte runtimes', error);
      });
      return;
    }
